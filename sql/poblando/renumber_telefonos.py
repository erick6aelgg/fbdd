#!/usr/bin/env python3
"""
Renumber first numeric field in VALUES(...) for INSERTs into Telefonos (or all INSERTs in file)
Usage: renumber_telefonos.py /path/to/Telefonos.sql start_id end_id
Creates a backup `Telefonos.sql.bak` before overwriting.
"""
import sys
import re
...
def main():
    if len(sys.argv) < 4:
        print("Usage: renumber_telefonos.py /path/to/Telefonos.sql start_id end_id [tableName]")
        sys.exit(2)
    p = Path(sys.argv[1])
